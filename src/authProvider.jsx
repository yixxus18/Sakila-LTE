...
const PERMISSIONS = {
  ...
  [ROLES.CUSTOMER]: {
    read: ['films', 'categories', 'actors', 'languages', 'inventory', 'rentals', 'payments'], 
...
const AuthProvider = ({ children }) => {
      ...
      if (storedToken && storedUserData) {
        try {
          ...
          if (userData && userData.id && userData.role_id) {
            ...
            console.warn("AuthProvider: Invalid user data found in cookies. Clearing session.");
            handleLogout(); 
          }
        } catch (error) {
          console.error("AuthProvider: Error parsing user data from cookies. Clearing session.", error);
          handleLogout(); 
        }
      } else {
        setUserState(null);
        setRoleState(ROLES.GUEST);
        if (!storedToken) destroyCookie(null, authConfig.storageTokenKeyName, { path: "/" });
        if (!storedUserData) destroyCookie(null, storageUserKeyName, { path: "/" });
  ...
  const handleLogin = async (params, errorCallback) => {
    try {
      ...
      if (response?.message) {
        ...
        console.warn("AuthProvider: Login response missing 'message'. Assuming success for navigation.");
      ...
      if (errorCallback) {
          errorCallback(message.includes("401") ? "Credenciales inválidas" : message);
      }
      throw err;
    }
  };
  ...
  const handleVerifyCode = async (email, code, errorCallback) => {
    ...
    try {
      ...
      const response = await postData(sakilaConfig.auth.twofactorauth, verificationData, {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      });
      ...
      if (token && userData && userData.id && userData.role_id) {
        ...
        console.log("AuthProvider: Verification successful. User:", userData.id, "Role:", userData.role_id);
        setLoading(false);
        return response; 
      } else {
        console.error("AuthProvider: Verification response missing token or valid user data.", response);
        const message = response?.message || "Código de verificación inválido o respuesta inesperada.";
      ...
      const message = err?.response?.data?.message || err?.message || "Error al verificar el código.";
       if (errorCallback) errorCallback(message);
      setLoading(false);
      throw err; 
    } finally {
